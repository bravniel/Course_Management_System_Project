export const newStudentFormInitialState = {
  values: {
    firstName: "",
    lastName: "",
    birthDate: "",
...
export function NewStudentFormReducer(state, action) {
  console.log(state);
  console.log(action);
  switch (action.type) {
    case "FIRST_NAME": {
    ...
    case "BIRTH_DATE": {
      const newValues = { ...state.values, birthDate: action.payload.value };
      console.log(newValues);
      const newIsValid = {
        ...state.isValid,
        birthDate: action.payload.isValidInput,
      };
      console.log(newIsValid);
      const newIsFormValid = IsFormValid({
        isValid: { ...newIsValid },
...
function IsFormValid(state) {
  return (
    ...
    state.values.birthDate.length > 0 &&
